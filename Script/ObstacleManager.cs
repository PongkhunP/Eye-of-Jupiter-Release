...
public partial class ObstacleManager : Node2D
{
    ...
    private void SpawnHazard(Vector2I clusterOrigin, List<Vector2I> tiles)
    {
        ...
        float zoneRadius = Mathf.Max(tileSize.X, tileSize.Y) * ClusterSize * 0.5f;
 
        // Random tile in cluster + random sub-tile offset for natural placement
        Vector2I randomTile = tiles[GD.RandRange(0, tiles.Count - 1)];
        Vector2 spawnPos = HazardLayer.MapToLocal(randomTile) + new Vector2(
            (GD.Randf() - 0.5f) * tileSize.X,
            (GD.Randf() - 0.5f) * tileSize.Y
        );
 
        var zone = new HazardZone();
        zone.O2DrainPerSecond = HazardO2Drain;
        zone.HpDrainPerSecond = HazardHpDrain;
 
        var shape = new CollisionShape2D();
        shape.Shape = new CircleShape2D { Radius = zoneRadius };
        zone.AddChild(shape);
 
        zone.GlobalPosition = spawnPos;
        AddChild(zone);
 
        _spawnedZones[clusterOrigin] = zone;
    }
